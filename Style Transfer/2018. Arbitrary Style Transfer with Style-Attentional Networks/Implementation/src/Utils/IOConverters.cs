    ...
    public static class IOConverters
    {
        ...
        public static unsafe Bitmap TensorToImage(Tensor img)
        {
            ...
            for(int y = 0; y < img.Height; y++)
            {
                ...
                {
                    var r = Math.Min(Math.Max(img.Get(x, y, 0), 0f), 1f) * 255.0f;
                    var g = Math.Min(Math.Max(img.Get(x, y, 1), 0f), 1f) * 255.0f;
                    var b = Math.Min(Math.Max(img.Get(x, y, 2), 0f), 1f) * 255.0f;
                    *Addr = (byte)b;
                    Addr += 1;
                    *Addr = (byte)g;
                    Addr += 1;
