...
namespace NeuralArt
{
    ...
    public static partial class Layers
    {
        ...
        ///<param name="alpha">Глубина стилизации. [0..1], по умолчанию = 1.</param>
        ///<param name="epsilon">Системный параметр, защищающий от деления на 0. По умолчанию = 0.00001.</param>
        public static Tensor AdaIN(Tensor Content, Tensor Style, float alpha = 1.0f, float epsilon = 1e-5f)
        {
            ...
            Parallel.For(0, Content.Depth, (int d) =>
            {
                for(int y = 0; y < Content.Height; y++)
                {
                    for(int x = 0; x < Content.Width; x++)
                    {
                        Normalized.Set(x, y, d, alpha * Normalized.Get(x, y, d) + (1 - alpha) * Content.Get(x, y, d));
                    }
                }
            });
            return Normalized;
