...
//-> Методы преобразования System.Drawing.Bitmap <=> Waifu2x.Tensor.
    ...
    public static class IOConverters
    {
        ...
        public static unsafe Bitmap TensorToImage(Tensor img)
        {
            ...
            for(int y = 0; y < img.Height; y++)
            {
                ...
                {
                    var r = img.Get(x, y, 0) * 255.0f;
                    var g = img.Get(x, y, 1) * 255.0f;
                    var b = img.Get(x, y, 2) * 255.0f;
                    r = Math.Min(Math.Max(r, 0f), 255f);
                    g = Math.Min(Math.Max(g, 0f), 255f);
                    b = Math.Min(Math.Max(b, 0f), 255f);
                    *Addr = (byte)b;
                    Addr += 1;
                    *Addr = (byte)g;
                    Addr += 1;
